...
filename = 'test_im1.bmp'
#filename = 'test_blerta.bmp'


img=io.imread(filename)
imgFrom = img[18:211,19:298]
img = np.pad(img,pad_width=padding, mode='constant', constant_values=0)

for ii in range(211+padding,240+padding):
    for jj in range(19+padding,img.shape[1]-2*padding-2):
        img[ii,jj] = find_pixel_value(ii,jj,imgFrom, img, window_size)
for ii in reversed(range(0+padding,19+padding)):#180+padding   154
    for jj in range(19+padding,img.shape[1]-2*padding-2):
        img[ii,jj] = find_pixel_value(ii,jj,imgFrom, img, window_size)

for jj in reversed(range(0+padding,19+padding)):
    for ii in range(padding,img.shape[0]-padding):#180+padding   154
        img[ii,jj] = find_pixel_value(ii,jj,imgFrom, img, window_size)
for jj in range(298+padding,img.shape[1]-padding):
    for ii in range(padding,img.shape[0]-padding):#180+padding   154
        img[ii,jj] = find_pixel_value(ii,jj,imgFrom, img, window_size)
...
scipy.misc.imsave('city_11.jpg', bl)
